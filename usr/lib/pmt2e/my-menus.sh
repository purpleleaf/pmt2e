...
if [[ $1 == --weather ]]; then
    if [[ ! -f ~/.t2ecol/menu-weather.sh ]]; then
        cp /usr/lib/t2ec/menu-template.sh ~/.t2ecol/menu-weather.sh
    fi
    exec ~/.t2ecol/menu-weather.sh
fi

if [[ $1 == --battery ]]; then
    if [[ ! -f ~/.t2ecol/menu-battery.sh ]]; then
        cp /usr/lib/t2ec/menu-template.sh ~/.t2ecol/menu-battery.sh
    fi
    exec ~/.t2ecol/menu-battery.sh
fi
