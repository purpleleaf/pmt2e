...
3. enter both values in the ~/.config/pmt2e/weatherrc file;
...
Arguments to override some ~/.config/pmt2e/weatherrc config file values:
...
def main():
    ...
    response = None
    name = None
    img_path = "/usr/share/pmt2e/icons/"
    glyphs = None
    ...
    if len(sys.argv) > 1:
        for i in range(1, len(sys.argv)):
            ...
            if sys.argv[i].startswith("-D"):
                c_id = sys.argv[i][2::]
                if c_id:
                    show_details(pmt2e_dir, c_id)
                    exit
                else:
                    show_details(pmt2e_dir, settings.city_id)
                    exit
    ...
    if settings.glyphs is not None:
    ...
    if response is not None:
        ...
        if owm.cod == 200:
            # Prepare panel items
            icon = "/usr/share/pmt2e/icons/refresh.svg"
            try:
                icon_name = img_path + icons_name[str(getattr(owm.weather[0], "icon"))]
            except KeyError:
                pass
            ...
            if name is None:
                if glyphs:
                    font_icon = font_icons[str(getattr(owm.weather[0], "icon"))]
                    print(font_icon)
                    sys.exit()
                else:
                    os.system("echo " + icon_name)
            else:
                output += name
            ...
            subprocess.call(["echo '" + str(details) + "' > " + pmt2e_dir + "/.weather-" + settings.city_id], shell=True)
...
def show_details(pmt2e_dir, city):
    details = ""
    try:
        details = open(pmt2e_dir + "/.weather-" + city, 'r').read().rstrip().splitlines()
...
def print_help():
    ...
    print("\nFor multiple executors you may override /home/user/.config/pmt2e/weatherrc settings with arguments:")
    print( "pmt2e --weather [-I<items>] [-A<api_key>] [-C<city_id>] [-U<units>] [-L<lang>]")
    print(" <items>: [i]icon [s]hort description, [d]escription, [t]emperature, [p]ressure, [h]umidity, [w]ind, [c]ity name, [S]unrise/sunset")
    print(" Check 'Show icon' and set 'Continous output' to 2 in Tint2 settings\n")
    print("\nShow details as a notification:")
    print(" pmt2e --weather -D[<city_id>]")
    print(" Add [<city_id>] if varies from weatherrc city_id field.")
    print("\nShow icon as font glyphs (requires a Nerd font of your choice):")
    print(" Uncheck 'Show icon' and set 'Continous output' to 1 in Tint2 settings")
    print(" pmt2e --weather -F")
    print("\n")
...
class Settings:
    def __init__(self):
        ...
        if not os.path.isfile(pmt2e_dir + "/weatherrc"):
            config = [
            ...
            subprocess.call(["echo '" + ''.join(config) + "' > " + pmt2e_dir + "/weatherrc"], shell=True)
        ...
        lines = open(pmt2e_dir + "/weatherrc", 'r').read().rstrip().splitlines()
        ...
        for line in lines:
            if not line.startswith("#"):
                if line.startswith("items"):
                    ...
                    self.glyphs = line.split("=")[1].strip()
