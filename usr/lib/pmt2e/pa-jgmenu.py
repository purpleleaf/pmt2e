...
def main():
    ...
    for output in outputs:
        jgmenu.append(output[1] + ", " + "/usr/lib/pmt2e/pa-switch-output.sh " + output[0])
    ...
    with open(pmt2e_dir + '/pulseaudio-menu.sh', 'w') as f:
        for item in jgmenu:
            f.write("%s\n" % item)

    # Make executable
    os.system("chmod u+x " + pmt2e_dir + "/pulseaudio-menu.sh")

    # Execute jgmenu script
    os.system(pmt2e_dir + "/pulseaudio-menu.sh &")
